...
export function MonthlyReservationsChart({ bookings, isLoading = false }: MonthlyReservationsChartProps) {
  ...
  const chartData = useMemo(() => {
    if (viewMode === 'monthly') {
      ...
      return processDailyData(bookings)
    }
  }, [bookings, viewMode])
  ...
  const averagePerPeriod = useMemo(() => {
    if (viewMode === 'monthly') {
      return chartData.labels.length > 0 ? Math.round(totalReservations / chartData.labels.length) : 0
    } else {
      return chartData.labels.length > 0 ? Math.round(totalReservations / chartData.labels.length) : 0
    }
  }, [chartData, totalReservations, viewMode])
    ...
    return (
      <Card className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>Trend reservasi bulanan</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <BarChart3 className="w-5 h-5" />
              {viewMode === 'monthly' ? 'Monthly Trends' : 'Daily Distribution'}
            </CardTitle>
            <CardDescription>
              {viewMode === 'monthly' ? 'Trend reservasi bulanan' : 'Distribusi reservasi harian'}
            </CardDescription>
          </div>
