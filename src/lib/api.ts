...
export interface TourBookingWithRelations extends TourBooking {
  profiles: {
    full_name: string
    email: string
    institution?: string
...
export const useToggleTourActive = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase
...
// Fetch all tour bookings with relations
export const useTourBookings = (filters?: {
  status?: string[]
  dateRange?: { start: string; end: string }
  search?: string
}) => {
  ...
  return useQuery<TourBookingWithRelations[]>({
    queryKey: ['tourBookings', filters],
    queryFn: async () => {
      let query = supabase
        .from('tour_bookings')
        .select(`
...
export const useUpdateTourBookingStatus = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase
