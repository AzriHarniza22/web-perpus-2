  ...
  return withAuth(request, async (req: AuthenticatedRequest) => {
    try {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: `Gagal mengambil data tour: ${error.message}` }, { status: 500 })
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (conflictError) {
      console.error('Tour conflict check error:', conflictError)
    ...
    const insertData = {
      user_id: user.id,
      room_id: tour_id, // tour_id stored in room_id field for compatibility
      start_time,
      end_time,
      event_description,
      proposal_file,
    ...
    try {
      console.log('About to send new tour booking notification to admin');
